...
export async function getLatest(collection) {
  ...
  return limiter.schedule(() => {
    return {
      ...
      saleDate: res.data.assets[0].last_sale.transaction.timestamp,
...
export async function getCollectionArtwork(collection, notable) {
    ...
    res.data.assets.map(async asset => {
      ...
      return limiter.schedule(() => {
        return {
          ...
          saleDate: asset.last_sale.transaction.timestamp,
  ...
  try {
    ...
    notableData = {
      ...
      saleDate: res3.data.last_sale.transaction.timestamp,
